...
- Codex account updates and Claude Code `rate_limit_event` messages now emit a
  shared structured `provider/rateLimits/updated` event. Provider window keys
  remain open strings, so future keys such as `seven_day_fable` survive without
  a bb schema release.
- The server exposes a conservative recovery inspection plus a transactionally
  guarded continuation. It requires accepted input, a terminal failed turn, no
...
- The default-enabled builtin Provider retry plugin owns the v1 in-memory
  ...
  for host reconnection, and clears all jobs on dispose.
- Timed windows recover automatically. Credit and spend-control blocks without
  reset times remain visible and support Refresh and Retry now without blind
  automatic retries.
- The composer banner, provider-retry CLI controls, guarded thread-retry CLI,
  SDK methods, realtime updates, CLI guide, and bb-cli skill are implemented.
  Restart durability remains intentionally excluded from v1.
